...
save_folder = os.path.join('results', 'cifar10', model_str, '%s_%s_%s_%s' % ("original", opt_str, decay_str, lambda_str))
...
for epoch in range(start_epoch, args.epochs):
    ...
    for i, batch in enumerate(train_loader):
        ...
        if i % 100 == 0:
            train_str = 'Epoch %d-%d | Train | Loss: %.4f, TA: %.4f' % (
                epoch, i, losses.avg, accs.avg)
            print(train_str)
            # print('idx2BN:', idx2BN)
    ...
    if eval_this_epoch:
        val_accs = {}
        for val_lambda in val_lambdas:
            val_accs[val_lambda] = AverageMeter()
    ...
    if eval_this_epoch:
        ...
        for val_lambda in val_lambdas:
            val_str += 'val_lambda%s: TA: %.4f\n' % (val_lambda, val_accs[val_lambda].avg)
    print(val_str)
    ...
    save_original_ckpt(epoch, model, optimizer, scheduler, best_TA, training_loss, val_TA, os.path.join(save_folder, 'latest.pth'))
